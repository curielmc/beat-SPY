  ...
  <div class="min-h-screen bg-base-200 flex">
    ...
    <aside class="fixed lg:sticky top-0 left-0 z-40 h-screen w-56 bg-base-100 border-r border-base-300 flex flex-col transition-transform lg:translate-x-0" :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'">
      ...
      <nav class="flex-1 p-3 space-y-1">
        ...
        <button class="btn btn-outline btn-sm justify-start w-full gap-2 border-primary/30 text-primary" :disabled="masquerading" @click="viewAsRandomStudent">
          <span v-if="masquerading" class="loading loading-spinner loading-xs"></span>
...
async function viewAsRandomStudent() {
  sidebarOpen.value = false
  masquerading.value = true
  try {
    // Find a random student who is in a class with a group
    const { data: memberships } = await supabase
      .from('class_memberships')
      .select('user_id, group_id, profiles:profiles(id, full_name, role)')
      .not('group_id', 'is', null)
    const students = (memberships || []).filter(m => m.profiles?.role === 'student' && m.profiles?.id)
    if (!students.length) {
      alert('No students found in any class.')
      return
    }
    const pick = students[Math.floor(Math.random() * students.length)]
    const result = await auth.startMasquerade(pick.profiles)
    if (!result?.error) {
      router.push('/home')
    }
  } finally {
