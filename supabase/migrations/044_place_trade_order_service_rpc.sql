-- Allow the server API to place trades on behalf of a validated user without
-- reopening direct client access to the pricing-sensitive RPC.

CREATE OR REPLACE FUNCTION place_trade_order_as_user(
  p_user_id uuid,
  p_portfolio_id uuid,
  p_ticker text,
  p_side text,
...
REVOKE ALL ON FUNCTION place_trade_order_as_user(uuid, uuid, text, text, numeric, numeric, text, text, numeric) FROM PUBLIC;
REVOKE ALL ON FUNCTION place_trade_order_as_user(uuid, uuid, text, text, numeric, numeric, text, text, numeric) FROM authenticated;
REVOKE ALL ON FUNCTION place_trade_order_as_user(uuid, uuid, text, text, numeric, numeric, text, text, numeric) FROM anon;
GRANT EXECUTE ON FUNCTION place_trade_order_as_user(uuid, uuid, text, text, numeric, numeric, text, text, numeric) TO service_role;
